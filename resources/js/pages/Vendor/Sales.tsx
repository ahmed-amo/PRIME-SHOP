...
export default function VendorSales({ products }: Props) {
  const { t } = useI18n();
  ...
  useEffect(() => {
    ...
    products.data.forEach((p) => {
      const onCompareSale = p.compare_at_price != null && p.compare_at_price > p.price;
      next[p.id] = {
  ...
  const saveRow = (product: ProductRow) => {
    ...
    router.patch(
      route("vendor.sales.update", product.id),
      {
    ...
    <div className="space-y-6">
      {flashMessage && (
        <Alert className="border-green-200 bg-green-50 dark:border-green-900 dark:bg-green-950/40">
          <AlertDescription className="text-green-800 dark:text-green-200">{flashMessage}</AlertDescription>
        </Alert>
      )}
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
            {t(
              "First column is the list/original price. Enter the new sale price in the second column (empty until you add a promotion). Original must be higher than sale price.",
            )}
          </p>
        </div>
      ...
      <Card>
        <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <CardTitle>
            {t("Products")} ({filtered.length} / {products.total})
          </CardTitle>
        ...
        <CardContent>
          {filtered.length === 0 ? (
            <div className="py-16 text-center text-muted-foreground">{t("No products found.")}</div>
          ) : (
