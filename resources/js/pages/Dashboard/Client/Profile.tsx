...
export default function ProfilePage({ user, mustVerifyEmail, status }: ProfilePageProps) {
  ...
  const avatarSrc = useMemo(() => {
    return getAvatarUrl(user?.picture ?? user?.avatar) ?? undefined;
          ...
          <Card className="lg:col-span-1">
            ...
            <CardContent className="flex flex-col items-center gap-4">
              <div className="relative">
                ...
                <Button size="icon" className={`absolute bottom-0 ${isRtl ? 'left-0' : 'right-0'} rounded-full h-10 w-10`} type="button">
              ...
              <div className="space-y-6">
                ...
                <div className="flex gap-4">
                  <Button onClick={handleSubmit} className="flex-1 bg-orange-600 hover:bg-orange-700" disabled={processing}>
                    {processing && <Loader2 className={`${isRtl ? 'ml-2' : 'mr-2'} h-4 w-4 animate-spin`} />}
                    Save Changes
                  </Button>
