...
export default function CheckoutPage({ auth }: CheckoutPageProps) {
  ...
  const inputCls = (field: string) =>
    `h-11 text-gray-900 placeholder:text-gray-400 bg-white border-gray-200 focus:ring-1 rounded-xl [color-scheme:light] transition-colors ${
      errors[field] ? "border-red-400 focus:border-red-400 focus:ring-red-200" : "focus:border-orange-400 focus:ring-orange-400"
    }`
              ...
              <div className="space-y-5">
                ...
                {errors.payment_method && (
                  <div className="flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-xl">
                    ...
                    <p className="text-red-600 text-sm font-medium">{errors.payment_method}</p>
                  </div>
                )}
              ...
              <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    ...
                    <div key={item.id} className="flex gap-4 pb-4 border-b border-gray-100 last:border-0 last:pb-0">
                      ...
                      <div className="flex-1 min-w-0 py-1">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          ...
                          <span className="text-xs text-gray-400">{item.price.toFixed(2)} DA each</span>
                        </div>
                        <p className="text-sm font-bold text-gray-800 mt-2">{(item.price * item.quantity).toFixed(2)} <span className="text-xs font-normal text-gray-400">DA</span></p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="px-6 py-4 border-t border-gray-100 space-y-2.5 bg-gray-50">
                  <div className="flex justify-between text-sm text-gray-500">
                    <span>Subtotal</span>
                    <span className="font-semibold text-gray-900">{subtotal.toFixed(2)} DA</span>
                  </div>
                  <div className="flex justify-between text-sm text-gray-500">
                    <span>Shipping</span>
                    <span className="font-semibold text-gray-900">{shippingCost.toFixed(2)} DA</span>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-orange-500 to-rose-500 px-6 py-5">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-white/80 text-sm font-medium">Total to Pay</span>
                    <span className="text-white text-2xl font-extrabold tracking-tight">{total.toFixed(2)} DA</span>
                  </div>
                  <div className="flex items-center gap-4 mt-3">
                    <div className="flex items-center gap-1.5">
                      <ShieldCheck className="w-3.5 h-3.5 text-white/90" />
                      <span className="text-white/90 text-xs font-medium">Protected</span>
                    </div>
                    <div className="w-px h-3 bg-white/30" />
                    <div className="flex items-center gap-1.5">
                      <Zap className="w-3.5 h-3.5 text-white/90" />
                      <span className="text-white/90 text-xs font-medium">Fast delivery 24h</span>
                    </div>
                  </div>
                </div>
