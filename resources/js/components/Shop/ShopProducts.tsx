...
function ShopProductsInner({ products }: { products: Product[] | { data: Product[] } }) {
  ...
  function ProductCard({ product, index }: { product: Product; index: number }) {
    const images = useMemo(() => {
      ...
      const one = product.image_url ?? placeholder;
      return [one];
    }, [product.images, product.image_url]);

    const [imgIndex, setImgIndex] = useState(0);
    const hasGallery = images.length > 1;
    const currentSrc = images[Math.min(imgIndex, images.length - 1)] ?? placeholder;

    const prev = (e: React.MouseEvent) => {
      e.preventDefault();
      e.stopPropagation();
      if (!hasGallery) return;
      setImgIndex((v) => (v - 1 + images.length) % images.length);
    };
    const next = (e: React.MouseEvent) => {
      e.preventDefault();
      e.stopPropagation();
      if (!hasGallery) return;
      setImgIndex((v) => (v + 1) % images.length);
    };
      ...
      >
        <Link href={`/product/${product.slug ?? product.id}`} className="relative block">
          ...
          <img
            ...
            loading="lazy"
            decoding="async"
            className="h-40 w-full object-cover transition-transform duration-300 group-hover:scale-105 sm:h-56"
            onError={(e) => {
          ...
          {hasGallery && (
            <>
              <button
                ...
                onClick={prev}
                className="absolute start-2 top-1/2 z-20 -translate-y-1/2 rounded-full bg-white/85 p-1.5 text-gray-900 shadow ring-1 ring-black/5 transition hover:bg-white focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-500"
              >
              ...
              <button
                ...
                onClick={next}
                className="absolute end-2 top-1/2 z-20 -translate-y-1/2 rounded-full bg-white/85 p-1.5 text-gray-900 shadow ring-1 ring-black/5 transition hover:bg-white focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-500"
              >
