...
export default function CategoryOne({ categories = [] }: { categories?: CategoryItem[] }) {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      else if (window.innerWidth < 1280) setVisibleItems(5);
  ...
  return (
    <div className="w-full overflow-hidden bg-gradient-to-b from-amber-50/70 to-amber-50/30 py-8 px-4 md:px-8 relative border-y border-amber-100/50">
      <div className="max-w-7xl mx-auto">
        ...
        <div className={`flex items-center justify-between mb-8 transition-opacity duration-1000 ${isVisible ? "opacity-100" : "opacity-0"}`}>
          <div>
          ...
          <div className="flex space-x-3 m-10">
            <button onClick={goToPrev} disabled={sliding} className="p-2.5 rounded-full bg-white border border-amber-200 text-gray-600 hover:bg-amber-50 hover:border-amber-300 transition-all shadow-sm disabled:opacity-50 hover:scale-105 active:scale-95">
              <ChevronLeft className="h-5 w-5" />
            </button>
            <button onClick={goToNext} disabled={sliding} className="p-2.5 rounded-full bg-white border border-amber-200 text-gray-600 hover:bg-amber-50 hover:border-amber-300 transition-all shadow-sm disabled:opacity-50 hover:scale-105 active:scale-95">
          ...
          <div className={`flex transition-transform duration-500 ease-in-out ${isVisible ? "opacity-100" : "opacity-0"}`} style={{ transform: `translateX(0%)`, width: "100%", transitionDelay: "0.2s" }}>
            <div className="w-full grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 md:gap-10">
              {visibleCategories().map((category, idx) => (
                ...
                >
                  <div
                    className={`w-full aspect-square rounded-full overflow-hidden p-1.5 shadow-sm transition-all duration-300 transform ${hoveredCategory === category.id ? "scale-110 shadow-lg" : ""}`}
                    style={{ backgroundColor: hoveredCategory === category.id && category.color ? category.color : "transparent" }}
                  >
        ...
        <div className="flex justify-center items-center space-x-3 mt-8">
          {Array.from({ length: totalPages }).map((_, index) => (
            <button
              ...
              className={`h-2.5 rounded-full transition-all duration-300 ${activeIndex === index ? "w-10 bg-gradient-to-r from-amber-400 to-amber-500 shadow-sm" : "w-2.5 bg-gray-200 hover:bg-amber-200"}`}
        ...
        <div className={`flex justify-center mt-8 transition-opacity duration-1000 ${isVisible ? "opacity-100" : "opacity-0"}`} style={{ transitionDelay: "0.4s" }}>
          <a
            href="/categories"
